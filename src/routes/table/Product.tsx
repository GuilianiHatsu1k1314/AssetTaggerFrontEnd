...
const columns = [
  ...
  columnHelper.accessor("productName", {
    cell: (info) => {
      ...
      return val ? (
        ...
        <span className="text-gray-400 italic">Unnamed Product</span>
      );
    },
    header: "Product Name",
  }),
  columnHelper.accessor("productModelNumber", {
    cell: (info) => {
      const val = info.getValue();
      return val ? (
        <span className="font-mono text-sm text-gray-700">{val}</span>
      ) : (
  ...
  columnHelper.accessor("categoryId", {
    cell: (info) => (
      <span className="rounded border border-indigo-100 bg-indigo-50 px-2 py-1 font-mono text-sm font-semibold text-gray-600">
        {info.getValue()}
      </span>
    ),
    header: "Category ID",
  }),
  columnHelper.accessor("productInsertDate", {
...
function ProductPage() {
  ...
  const handleRowClick = (productId: string) => {
    if (isEditMode) {
      ...
      navigate({ search: { productId }, to: "/EditValue" as any });
      setIsEditMode(false);
    }
  };
    ...
    <div className="flex h-screen w-full flex-col bg-gray-50 md:flex-row">
      ...
      <main className="flex-1 overflow-y-auto p-4 pb-28 md:p-8">
        ...
        <div className="mb-6 flex items-center gap-2 text-sm font-medium text-gray-500 md:text-base">
          ...
          <span>/</span>
          <span className="text-gray-900">Product Display</span>
        </div>
        ...
        <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Manage global product catalogs, models, and classifications.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Link
              ...
              to="/AddValue" // Can redirect to specific Add Product form later
            >
              <svg
                fill="none"
                height="16"
              ...
              <tbody className="divide-y divide-gray-100 bg-white">
                {table.getRowModel().rows.map((row) => (
                  <tr
                    ...
                    key={row.id}
                    onClick={() => {
                      handleRowClick(row.original.productId);
                    }}
                  >
                ...
                {table.getRowModel().rows.length === 0 && (
                  <tr>
                    ...
                    >
                      No products found. Click "Add Product" to register one.
                    </td>
                  </tr>
                )}
              </tbody>
