...
const columns = [
  columnHelper.accessor("assetFixId", {
    cell: (info) => (
      <span className="font-semibold text-gray-900">{info.getValue()}</span>
    ),
    header: "Fix ID",
  }),
  columnHelper.accessor("assetFixTitle", {
    cell: (info) => (
      <span className="font-medium text-blue-600">{info.getValue()}</span>
    ),
    header: "Title",
  }),
  columnHelper.accessor("assetFixed", {
    cell: (info) => {
      const isFixed = info.getValue();
      return (
        <span
  ...
  columnHelper.accessor("assetFixDateStart", {
    cell: (info) => info.getValue() || "-",
    header: "Date Start",
    sortingFn: "datetime",
  }),
  columnHelper.accessor("assetFixDateEnd", {
    cell: (info) => info.getValue() || "-",
    header: "Date End",
    sortingFn: "datetime",
  }),
  columnHelper.accessor("assetFixCost", {
    cell: (info) => {
  ...
  columnHelper.accessor("assetIssueId", {
    ...
    <div className="flex h-screen w-full flex-col bg-gray-50 md:flex-row">
      ...
      <main className="flex-1 overflow-y-auto p-4 pb-28 md:p-8">
        ...
        <div className="mb-6 flex items-center gap-2 text-sm font-medium text-gray-500 md:text-base">
          ...
          <span>/</span>
          <span className="text-gray-900">Asset Fix Display</span>
        </div>
        ...
        <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Manage repair logs, costs, and fix statuses.
            </p>
          </div>
            ...
            <div className="relative">
              ...
              {isFilterOpen && (
                <div className="absolute top-full right-0 z-50 mt-2 w-56 rounded-lg border border-gray-200 bg-white py-2 shadow-xl">
                  ...
                  <SortMenuItem
                    columnId="assetFixDateStart"
                    label="Start Date"
                    table={table}
                  />
                  <SortMenuItem
                    columnId="assetFixDateEnd"
                    label="End Date"
                    table={table}
                  />
                  <SortMenuItem
                    columnId="assetFixCost"
                    label="Repair Cost"
                    table={table}
                  />
                  <SortMenuItem
              ...
              <tbody className="divide-y divide-gray-100 bg-white">
                {table.getRowModel().rows.map((row) => (
                  <tr
                    ...
                    key={row.id}
                    onClick={() => {
                      handleRowClick(row.original.assetFixId);
                    }}
                  >
                ...
                {table.getRowModel().rows.length === 0 && (
                  <tr>
                    ...
                    >
                      No asset fix logs found. Click "Add Fix Log" to create
                      one.
                    </td>
                  </tr>
                )}
