...
// Import the hook and interface from your context
import { type EndUserRole, useDatabase } from "../context/-AssetContext";

export const Route = createFileRoute("/table/EndUserRole")({
  component: EndUserRolePage,
});

const columnHelper = createColumnHelper<EndUserRole>();

// ==========================================
// TABLE COLUMNS (High-Level Overview)
// ==========================================
const columns = [
  columnHelper.accessor("endUserRoleName", {
    cell: (info) => (
      <span className="font-bold text-blue-600">
        {info.getValue() || "Unnamed Role"}
      </span>
    ),
    header: "End User Role Name",
  }),
  columnHelper.accessor("endUserRoleId", {
    cell: (info) => (
      <span className="font-mono text-sm text-gray-500">{info.getValue()}</span>
    ),
    header: "End User Role ID",
  }),
  columnHelper.accessor("endUserRoleCreationDate", {
    cell: (info) => {
...
function EndUserRolePage() {
  const navigate = useNavigate();

  // Pull live data specifically for the EndUserRole table!
  const { currentUser, getTableData } = useDatabase();
  const roles = getTableData("EndUserRole");

  // Determine if the active user is an Admin
  const isAdmin = currentUser?.roleName === "admin";

  const [isEditMode, setIsEditMode] = useState(() => {
    return sessionStorage.getItem("EndUserRole_isEditMode") === "true";
  });

  const toggleEditMode = () => {
    const newValue = !isEditMode;
    setIsEditMode(newValue);
    sessionStorage.setItem("EndUserRole_isEditMode", String(newValue));
  };
  ...
  const handleRowClick = (endUserRoleId: string) => {
    if (isEditMode) {
      navigate({
        search: { id: endUserRoleId, tableName: "EndUserRole" },
        to: "/EditValue" as any,
      });
    }
  };
      ...
      <div className="flex h-screen w-full flex-col bg-gray-50 md:flex-row">
        ...
        <main className="flex-1 overflow-y-auto p-4 pb-28 md:p-8">
          ...
          <div className="mb-6 flex items-center gap-2 text-sm font-medium text-gray-500 md:text-base">
            ...
            <span>/</span>
            <span className="text-gray-900"> End User Roles Display</span>
          </div>
          ...
          <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Configure security profiles and granular system permissions.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Link
                className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-medium text-white shadow-sm transition-colors hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                to={`/adminpages/CreateRole`}
              >
                <svg
                  fill="none"
                  height="16"
              ...
              <div className="relative">
                ...
                {isFilterOpen && (
                  <div className="absolute top-full right-0 z-50 mt-2 w-56 rounded-lg border border-gray-200 bg-white py-2 shadow-xl">
                    ...
                    <SortMenuItem
                      columnId="endUserRoleName"
                      label="Role Name (A-Z)"
                      table={table}
                    />
                    <SortMenuItem
                      columnId="endUserRoleCreationDate"
                      label="Creation Date"
                      table={table}
                    />
                  </div>
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {table.getRowModel().rows.map((row) => (
                    <tr
                      ...
                      key={row.id}
                      onClick={() => {
                        handleRowClick(row.original.endUserRoleId);
                      }}
                    >
                  ...
                  {table.getRowModel().rows.length === 0 && (
                    <tr>
                      ...
                      >
                        No roles configured yet. Click "Create Role" to start.
                      </td>
                    </tr>
                  )}
                </tbody>
