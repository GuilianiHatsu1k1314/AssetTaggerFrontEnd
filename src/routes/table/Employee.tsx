...
import { type Employee, useDatabase } from "../context/-AssetContext";

export const Route = createFileRoute("/table/Employee")({
  component: EmployeePage,
});

// FIX: Corrected column helper type from Department to Employee
const columnHelper = createColumnHelper<Employee>();

// 2. COLUMNS CONFIGURED FOR SQL SCHEMA (Assuming these match your Employee type)
const columns = [
  // I updated these columns to match the Employee interface from your context
  ...
  columnHelper.accessor("employeeInsertDate", {
...
function EmployeePage() {
  ...
  const [isEditMode, setIsEditMode] = useState(() => {
    return sessionStorage.getItem("Employee_isEditMode") === "true"; // FIX: updated key
  });

  const toggleEditMode = () => {
    const newValue = !isEditMode;
    setIsEditMode(newValue);
    sessionStorage.setItem("Employee_isEditMode", String(newValue)); // FIX: updated key
  };
  // ==========================================
  ...
  const handleRowClick = (employeeId: string) => {
    if (isEditMode) {
      navigate({
        search: { id: employeeId, tableName: "Employee" }, // FIX: updated tableName
        to: "/EditValue" as any,
      });
      ...
      <div className="flex h-screen w-full flex-col bg-gray-50 md:flex-row">
        ...
        <main className="flex-1 overflow-y-auto p-4 pb-28 md:p-8">
          ...
          <div className="mb-6 flex items-center gap-2 text-sm font-medium text-gray-500 md:text-base">
            ...
            <span>/</span>
            <span className="text-gray-900">Employee Display</span>
          </div>
          ...
          <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Manage staff records, roles, and department assignments.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Link
                ...
                to={`/AddValue?tableName=Employee`} // <-- Updated to Employee!
              >
                <svg
                  fill="none"
                  height="16"
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {table.getRowModel().rows.map((row) => (
                    <tr
                      ...
                      key={row.id}
                      onClick={() => {
                        handleRowClick(row.original.employeeId);
                      }}
                    >
                  ...
                  {table.getRowModel().rows.length === 0 && (
                    <tr>
                      ...
                      >
                        No employees found. Click "Add Employee" to register
                        one.
                      </td>
                    </tr>
                  )}
