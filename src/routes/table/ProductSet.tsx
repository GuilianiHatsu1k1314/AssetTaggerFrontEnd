...
const columns = [
  columnHelper.accessor("parentProductId", {
    cell: (info) => (
      <span className="rounded border border-indigo-200 bg-indigo-50 px-2 py-1 font-mono text-sm font-semibold text-indigo-700">
        {info.getValue()}
      </span>
    ),
    header: "Parent Product ID (Bundle)",
  }),
  columnHelper.accessor("productId", {
    cell: (info) => (
      <span className="rounded border border-gray-200 bg-gray-100 px-2 py-1 font-mono text-sm font-medium text-gray-700">
        {info.getValue()}
      </span>
    ),
    header: "Child Product ID (Item)",
  }),
  columnHelper.accessor("productSetInsertDate", {
