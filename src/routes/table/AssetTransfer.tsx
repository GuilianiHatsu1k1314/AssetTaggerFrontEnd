...
const columns = [
  columnHelper.accessor("assetTransferId", {
    cell: (info) => (
      <span className="font-semibold text-gray-900">{info.getValue()}</span>
    ),
    header: "Transfer ID",
  }),
  columnHelper.accessor("assetTransferDate", {
  ...
  columnHelper.accessor("assetId", {
    cell: (info) => (
      <span className="rounded bg-gray-100 px-2 py-1 font-mono text-sm text-gray-600">
        {info.getValue()}
      </span>
    ),
    header: "Asset ID",
  ...
  columnHelper.accessor("companyId", {
    cell: (info) => (
      <span className="font-medium text-gray-600">{info.getValue()}</span>
    ),
    header: "From (Company ID)",
  }),
  columnHelper.accessor("receivingCompanyId", {
    cell: (info) => (
      <span className="font-medium text-blue-600">{info.getValue()}</span>
    ),
    header: "To (Company ID)",
  }),
  columnHelper.accessor("assetTransferPrice", {
    cell: (info) => {
      const val = info.getValue();
      return val ? `₱${val}` : <span className="text-gray-400">₱0</span>;
    },
      ...
      <div className="flex h-screen w-full flex-col bg-gray-50 md:flex-row">
        ...
        <main className="flex-1 overflow-y-auto p-4 pb-28 md:p-8">
          ...
          <div className="mb-6 flex items-center gap-2 text-sm font-medium text-gray-500 md:text-base">
            ...
            <span>/</span>
            <span className="text-gray-900">Asset Transfer Display</span>
          </div>
          ...
          <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Manage asset movements between companies and branches.
              </p>
            </div>
              ...
              <div className="relative">
                ...
                {isFilterOpen && (
                  <div className="absolute top-full right-0 z-50 mt-2 w-56 rounded-lg border border-gray-200 bg-white py-2 shadow-xl">
                    ...
                    <SortMenuItem
                      columnId="assetTransferDate"
                      label="Transfer Date"
                      table={table}
                    />
                    <SortMenuItem
                      columnId="assetTransferPrice"
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {table.getRowModel().rows.map((row) => (
                    <tr
                      ...
                      key={row.id}
                      onClick={() => {
                        handleRowClick(row.original.assetTransferId);
                      }}
                    >
                  ...
                  {table.getRowModel().rows.length === 0 && (
                    <tr>
                      ...
                      >
                        No asset transfers found. Click "Record Transfer" to log
                        one.
                      </td>
                    </tr>
                  )}
