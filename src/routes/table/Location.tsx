...
const columns = [
  ...
  columnHelper.accessor("locationAddress", {
    ...
    header: "Specific Address / Room",
  }),
  columnHelper.accessor("buildingId", {
    cell: (info) => (
      <span className="rounded border border-gray-200 bg-gray-100 px-2 py-1 font-mono text-sm font-semibold text-gray-600">
        {info.getValue()}
      </span>
    ),
    header: "Building ID",
  }),
  columnHelper.accessor("locationInsertDate", {
...
function LocationPage() {
  ...
  const handleRowClick = (locationId: string) => {
    if (isEditMode) {
      ...
      navigate({ search: { locationId }, to: "/EditValue" as any });
      setIsEditMode(false);
    }
  };
    ...
    <div className="flex h-screen w-full flex-col bg-gray-50 md:flex-row">
      ...
      <main className="flex-1 overflow-y-auto p-4 pb-28 md:p-8">
        ...
        <div className="mb-6 flex items-center gap-2 text-sm font-medium text-gray-500 md:text-base">
          ...
          <span>/</span>
          <span className="text-gray-900">Location Display</span>
        </div>
        ...
        <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Manage specific rooms, floors, or zones within a building.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Link
              ...
              to="/AddValue" // Can redirect to specific Add Location form later
            >
              <svg
                fill="none"
                height="16"
              ...
              <tbody className="divide-y divide-gray-100 bg-white">
                {table.getRowModel().rows.map((row) => (
                  <tr
                    ...
                    key={row.id}
                    onClick={() => {
                      handleRowClick(row.original.locationId);
                    }}
                  >
                ...
                {table.getRowModel().rows.length === 0 && (
                  <tr>
                    ...
                    >
                      No locations found. Click "Add Location" to register one.
                    </td>
                  </tr>
                )}
              </tbody>
